...
static int64_t button_press_timestamp = 0;
...
void button_pressed(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
	...
	if (gpio_pin_get_dt(&button)) {
		button_press_timestamp = k_uptime_get(); 
		msg.evt = BUTTON_EVT_PRESSED;
		printk("Button pressed at %" PRIu32 "\n", k_cycle_get_32());
		zbus_chan_pub(&chan_button_evt, &msg, K_NO_WAIT);
	} else {
		int64_t button_release_timestamp = k_uptime_get();
		int64_t button_press_duration = button_release_timestamp - button_press_timestamp; 
